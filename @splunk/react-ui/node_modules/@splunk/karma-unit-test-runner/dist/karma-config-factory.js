...
module.exports = function buildConfig(config, settings) {
    ...
    var runnerLocalPlugins = [];
    try {
        var runnerModulesPath = path.join(__dirname, '..', 'node_modules');
        runnerLocalPlugins = fs.readdirSync(runnerModulesPath).filter(function (plugin) {
        ...
        fileFilter.forEach(function (filter) {
            ...
            if (specificForApp) {
                if (specificForApp === app) {
                    ...
                    var filterSets = safeLoad(path.join(testApps[app].path, testApps[app].filtersets || 'filtersets.js'));
